...
//verify connection
app.listen(port, () => {
    console.log("App listening at http://%s:%s/plants", host, port);
});

//Read - get product list
app.get("/plants", async (req, res) => {
    try {
        const results = await db.collection("plants_catalog").find({}).toArray();
...
app.get("/plants/:id", async (req, res) => {
    ...
    console.log("Item to find: ", productid);
    await client.connect();
    console.log("Node connected successfully to GET-id MongoDB");
    const query = { "id": productid };
    const results = await db.collection("plants_catalog").findOne(query);
...
app.post("/plants/", async (req, res) => {
    try {
        ...
        const productExists = await db.collection("plants_catalog").findOne({ id: itemId });
        if (productExists) {
            // Item exists
        ...
        await db.collection("plants_catalog").insertOne({ id, title, price, description, image, rating });
...
app.put("/plants/:id", async (req, res) => {
    try {
        ...
        const filter = { id: id };
        const updateDoc = {
            $set: { title: title, price: price, description: description, image: image, rating: rating }
        };
        const options = { upsert: true };
        const result = await db.collection("plants_catalog").updateOne(filter, updateDoc, options);
...
app.delete("/plants/:id", async (req, res) => {
    try {
        const id = Number(req.params.id);
        const deleteResult = await db.collection("plants_catalog").deleteOne({ id: id });
